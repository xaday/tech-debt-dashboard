...
def generate_pdf(assessment: dict, results: dict) -> bytes:
    ...
    kpi_data = [
        ["Metric", "Value"],
        ["Total Annual Tech Debt Interest", f"{interest:.1f} €"],
        ["Total Application TCO", f"{tco:.1f} €/year"],
    ...
    story.append(Paragraph("Annual Interest Cost by Dimension (€)", heading_style))
    interest_dim = results.get("interest_by_dimension", {})
    interest_data = [["Dimension", "Annual Interest (€)"]]
    for dim, label in [("application", "Application"), ("infrastructure", "Infrastructure"),
                       ("architecture", "Architecture"), ("people", "People")]:
