    ...
    for idx, app in enumerate(apps):
        with st.expander(f"{'🟢' if app['name'] else '⚪'} {app['id']} — {app['name']}", expanded=(idx == 0)):
            ...
            with c2:
                ...
                app["dev_cost_per_resource"] = st.number_input("Dev Cost/Resource (€/year)", value=float(app["dev_cost_per_resource"]), min_value=0.0, key=f"app_dev_cost_{idx}")
                ...
                app["support_cost_per_resource"] = st.number_input("Support Cost/Resource (€/year)", value=float(app["support_cost_per_resource"]), min_value=0.0, key=f"app_sup_cost_{idx}")
                app["license_cost"] = st.number_input("License Cost (€/year)", value=float(app["license_cost"]), min_value=0.0, key=f"app_lic_{idx}")
                app["infra_cost"] = st.number_input("Infra Cost (€/year)", value=float(app["infra_cost"]), min_value=0.0, key=f"app_infra_{idx}")
                app["hardware_sw_cost"] = st.number_input("Hardware/SW Cost (€/year)", value=float(app["hardware_sw_cost"]), min_value=0.0, key=f"app_hw_{idx}")
    ...
    for idx, comp in enumerate(components):
        with st.expander(f"{'🟢' if comp['component_name'] else '⚪'} {comp['id']} — {comp['component_name']}", expanded=(idx == 0)):
            ...
            with c2:
                ...
                comp["engg_cost_per_resource"] = st.number_input("Engg Cost/Resource (€/year)", value=float(comp["engg_cost_per_resource"]), min_value=0.0, key=f"comp_engg_cost_{idx}")
                ...
                comp["support_cost_per_resource"] = st.number_input("Support Cost/Resource (€/year)", value=float(comp["support_cost_per_resource"]), min_value=0.0, key=f"app_sup_cost_c_{idx}")
                comp["license_cost"] = st.number_input("License Cost (€/year)", value=float(comp["license_cost"]), min_value=0.0, key=f"comp_lic_{idx}")
                comp["support_contract_cost"] = st.number_input("Support Contract (€/year)", value=float(comp["support_contract_cost"]), min_value=0.0, key=f"comp_contract_{idx}")
...
with tab_dashboard:
    ...
    c1, c2 = st.columns(2)
    c1.metric("Total Annual Interest (€)", f"{results['total_interest']:.1f}")
    c2.metric("Total TCO (€/year)", f"{results['total_tco']:.1f}")
    ...
    if results["app_details"]:
        st.markdown("#### Annual Interest Cost by Application (€)")
        ...
        bar_fig.update_layout(
            ...
            yaxis=dict(tickfont=dict(color="#FFFFFF"), title="€/year"),
    ...
    # ── TCO breakdown ─────────────────────────────────────────────────────────
    if results["app_details"]:
        st.markdown("#### TCO by Application (€/year)")
    ...
    st.markdown("#### Interest Cost by Dimension (€/year)")
